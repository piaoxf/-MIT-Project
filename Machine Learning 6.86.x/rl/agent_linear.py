...
def epsilon_greedy(state_vector, theta, epsilon):
    ...
    if np.random.rand() < epsilon:
        ...
        # shape (NUM_ACTIONS, NUM_OBJECTS)
        q_values = np.dot(theta, state_vector)

        # find the index of the maximum value in q_values
        max_index = np.argmax(q_values)
        # convert the index to a tuple (action_index, object_index)
...
def linear_q_learning(theta, current_state_vector, action_index, object_index,
    ...
    current_q = theta[index].dot(current_state_vector)

    # compute next Q value
    if terminal:
        target = reward
    else:
        # predict Q values for next state for all (a', b') pairs
        q_next_all = theta @ next_state_vector
        max_q_next = np.max(q_next_all)
        target = reward + GAMMA * max_q_next

    # compute TD error
    td_error = target - current_q

    # gradient step on theta for this (a, b) pair
    theta[index] += ALPHA * td_error * current_state_vector
